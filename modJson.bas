...
Option Explicit
' Native JSON parsing library for VB6 without referencing JS libraries <https://github.com/0xAA55/VB6JSON>
' Usage:
' Call the ParseJSONString() function or the ParseJSONString2() procedure to parse the JSON string, and get a returned Variant type variable
' Numeric types are parsed as Long or Currency, depending on the range of the value. Small values use Long, larger values use Currency, and if the value contains a decimal point or scientific notation, it uses Double.
' JSON strings are parsed as VB strings, with escape characters \ being properly escaped according to the specification.
' Lists [] are parsed as VB6 Variant arrays, and each array element can be of a different type.
' Objects {} are parsed as dictionaries (Scripting.Dictionary). Note that object variables need to be assigned using Set.
' The function JSONToString() does the opposite: it converts the parsed Variant back to a JSON string.
' ParseJSONString: Use the function return value to save the parsed Variant, and use "set varName=ParseJSONString(str)" or "varName=ParseJSONString(str)" depending on the target type.
' ParseJSONString2: Use the passed parameter to return the result, and it works correctly regardless of whether it is an object type. ParseJSONString2(str, retVariant)
' After getting the parsed result, how to determine the specific type of the Variant?
' First, determine if it is an object by using IsObject().
' If it is not an object, then determine if it is an array by using IsArray().
' If it is not an array, then determine if it is a string by using VarType() to get the Variant type number, and then check if the type number is vbString.
' You cannot directly use IsNumeric() because it will also consider numeric values stored in string format as numeric.
' If it is not a string, then it should be a numeric value. According to the VarType() return value, you can determine if it is Long, Currency, or Double. The type numbers are vbLong, vbCurrency, vbDouble respectively.
